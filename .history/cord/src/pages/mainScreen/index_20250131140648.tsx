...
import { useThreads, addThread, toggleLikeThread } from "@/hooks/useFirebase"; // Firebase hooks
...
export default function Main() {
  ...
  // いいねボタンの処理
  const handleLikeClick = async (threadId: string) => {
    try {
      const thread = threads.find((t) => t.id === threadId);
      if (thread) {
        await toggleLikeThread(thread);
      }
    } catch (error) {
